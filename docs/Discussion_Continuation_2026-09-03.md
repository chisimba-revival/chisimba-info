...
The next Announcements revision separates **Type**, **Audience**, **Scope**, **Presentation**, and **Updates delivery** in the publishing form. It stores What’s new, General announcement and Service notice types; everyone, administrator, `[-author-]` and `[-readonly-]` audiences; site or selected-class scope; one optional further-information URL; publication/expiry fields; and Latest-block eligibility. There is no separate summary: the short announcement message is the source, and the sidebar derives a bounded plain-text excerpt from it. Site publishing is server-restricted to administrators. Selected-class publishing requires administration privilege or `[-author-]` membership in every selected class, and class announcements are currently restricted to the General type.

Updates delivery occurs only when the publisher explicitly selects it. Announcements resolves active recipient user IDs by audience and selected class, excludes the publisher, and publishes one idempotent event through Notifications. No legacy email is sent. The new **What’s new for `[-authors-]`** sidebar block is visible only to administrators and people who teach at least one class; it shows up to three currently published, non-expired site items targeted to `[-authors-]`, including date, message excerpt and archive access.

Because Announcements has not been deployed elsewhere, unused compatibility fields were removed rather than migrated. The local publishing screen now uses a responsive Chisimba card form instead of the historical table layout. Chrome verifies a single Further information URL, clean class selection without exposed class codes, and separate Type, Audience, Scope, Presentation and Updates controls.

The publishing introduction now says, “Make an announcement to all site users, or to selected groups or `[-contexts-]`.” Scope uses one Site/Selected `[-contexts-]` selector instead of two radio controls, and the class checklist appears only for the selected-class scope. The title input uses the full available form width.
